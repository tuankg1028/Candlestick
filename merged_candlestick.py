...
MISSING_RATIOS = [0.6, 0.8, 0.95]  # 60%, 80%, 95% missing data (used for irregular data)

# Base directories for different experiments
BASE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "crypto_research_minute")
BASE_DIR_FULLIMAGE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "crypto_research_minute_fullimage")
BASE_DIR_IRREGULAR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "crypto_research_minute_irregular")

# Binance API data fetcher (standard version)
...
def fetch_coin_data_irregular(symbol, start_time, end_time, missing_ratio):
    url = "https://api.binance.com/api/v3/klines"
    all_data = []
    current_start = int(start_time.timestamp() * 1000)
    end_ms = int(end_time.timestamp() * 1000)
...
# Generate candlestick images and labels (standard version)
...
# Generate candlestick images with sparse windows for irregular data
def generate_images_irregular(df, window_size, output_dir, period_name, month_str, missing_ratio):
    os.makedirs(output_dir, exist_ok=True)
...
# Optimized image generation with batch processing
def generate_images_batch(df, window_size, output_dir, period_name, month_str, batch_size=100, missing_ratio=0):
    os.makedirs(output_dir, exist_ok=True)
    ...
    if missing_ratio > 0:
        # Use index as timestamps since it's set as index for irregular data
        original_timestamps = pd.date_range(start=df.index[0], end=df.index[-1], freq="1min")
        ...
        # Regular batch processing for complete data
        ...
        for batch_start in range(0, total_images, batch_size):
            ...
            for i in range(batch_start, batch_end):
                actual_index = i + window_size - 1
                window_df = df.iloc[actual_index - (window_size - 1):actual_index + 1]
                last_candle = window_df.iloc[-1]
                label = "UP" if last_candle["close"] > last_candle["open"] else "DOWN"
                image_path = f"candle_{actual_index}.png"
                batch_labels.append(label)
                batch_paths.append(image_path)
                
                # Use smaller figure size and lower DPI for faster generation
                fig, ax = plt.subplots(figsize=(1.5, 1.5))
                mpf.plot(window_df, type="candle", style="binance", ax=ax, 
                        axisoff=True, title="", ylabel="", xlabel="", volume=False)
                plt.tight_layout(pad=0)
                plt.savefig(os.path.join(output_dir, image_path), 
                            bbox_inches="tight", pad_inches=0, dpi=24)
                plt.close(fig)
            ...
            if batch_end % 1000 == 0:
                elapsed = time.time() - start_time
                speed = batch_end / elapsed if elapsed > 0 else 0
                print(f"Generated {batch_end}/{total_images} images for {month_str} 1m {period_name} w{window_size} ({speed:.2f} images/sec)")
    ...
    if valid_indices:
        ...
        print(f"No valid images generated for {month_str} 1m {period_name} w{window_size}")
        return None

# Load and preprocess images (standard version)
...
# Train CNN model (standard version)
def train_model(X, y, period_name, month_str, window_size, coin_dir, missing_ratio=0):
    if missing_ratio > 0:
...
def train_model_optimized(X, y, period_name, month_str, window_size, coin_dir, missing_ratio=0):
    if missing_ratio > 0:
        model_path = os.path.join(coin_dir, "models", f"model_{month_str}_1m_{period_name}_w{window_size}_{int(missing_ratio*100)}pct.h5")
    else:
        model_path = os.path.join(coin_dir, "models", f"model_{month_str}_1m_{period_name}_w{window_size}.h5")
    ...
    class_weights = compute_class_weight('balanced', classes=np.unique(y), y=y)
    history = model.fit(X, y, epochs=15, batch_size=64, class_weight=dict(enumerate(class_weights)),
...
def evaluate_and_save(model, X, y, period_name, month_str, window_size, coin_dir, dataset_type="train", exp_suffix="", missing_ratio=0):
    ...
    suffix_str = f" {missing_ratio*100}% {exp_suffix}" if missing_ratio > 0 else f" {exp_suffix}"
    
    with open(results_file, "w") as f:
        f.write(f"{dataset_type.capitalize()} Metrics for {month_str} 1m {period_name} w{window_size}{suffix_str}:\n")
...
def is_experiment_complete(symbol, train_month, test_months, window_size, missing_ratio=0, experiment_type="regular"):
    return False  # Always run experiments for merged functionality

# Main experiment runner for regular data
def run_experiments_for_coin_regular(symbol, train_month, test_months, window_size):
    ...
    for days in TIME_LENGTHS:
        ...
        labels_file = generate_images_batch(df, window_size, images_subdir, period_name, train_month_str)
        if labels_file:
            X, y = load_images_parallel(labels_file, images_subdir)
            if X is not None and len(X) > 0:
                model, history = train_model_optimized(X, y, period_name, train_month_str, window_size, coin_dir)
...
# Main experiment runner for full image data
def run_experiments_for_coin_fullimage(symbol, train_month, test_months, window_size):
    coin_dir = os.path.join(BASE_DIR_FULLIMAGE, symbol)
    RAW_DATA_DIR = os.path.join(coin_dir, "raw_data")
    IMAGES_DIR = os.path.join(coin_dir, "images")
    MODELS_DIR = os.path.join(coin_dir, "models")
    RESULTS_DIR = os.path.join(coin_dir, "results")
...
# Main experiment runner for irregular data
def run_experiments_for_coin_irregular(symbol, train_month, test_months, window_size, missing_ratio):
    coin_dir = os.path.join(BASE_DIR_IRREGULAR, symbol)
    RAW_DATA_DIR = os.path.join(coin_dir, "raw_data")
    IMAGES_DIR = os.path.join(coin_dir, "images")
    MODELS_DIR = os.path.join(coin_dir, "models")
    RESULTS_DIR = os.path.join(coin_dir, "results")
    ...
    for days in TIME_LENGTHS:
        ...
        if not os.path.exists(raw_file):
            df = fetch_coin_data_irregular(symbol, train_start, train_end, missing_ratio)
            df.set_index("timestamp", inplace=True)
            df.to_csv(raw_file)
            print(f"Raw data saved to {raw_file}")
        else:
        ...
        labels_file = generate_images_batch(df, window_size, images_subdir, period_name, train_month_str, missing_ratio=missing_ratio)
        if labels_file:
            X, y = load_images_parallel(labels_file, images_subdir)
            if X is not None and len(X) > 0:
...
def run_single_experiment(args):
    symbol, config, window_size, missing_ratio, experiment_type = args
    try:
        if experiment_type == "regular":
            run_experiments_for_coin_regular(symbol, config["train_month"], config["test_months"], window_size)
        elif experiment_type == "fullimage":
            run_experiments_for_coin_fullimage(symbol, config["train_month"], config["test_months"], window_size)
        elif experiment_type == "irregular":
            run_experiments_for_coin_irregular(symbol, config["train_month"], config["test_months"], window_size, missing_ratio)
        return f"Completed {symbol} w{window_size} {experiment_type} {missing_ratio*100 if missing_ratio else 0}%"
    except Exception as e:
        return f"Error in {symbol} w{window_size} {experiment_type}: {str(e)}"

# Run experiments in parallel across multiple coins/window sizes/ratios
def run_all_experiments_parallel(experiment_type="regular"):
    if experiment_type == "regular":
        os.makedirs(BASE_DIR, exist_ok=True)
    elif experiment_type == "fullimage":
        os.makedirs(BASE_DIR_FULLIMAGE, exist_ok=True)
    else:
        os.makedirs(BASE_DIR_IRREGULAR, exist_ok=True)
    ...
    for symbol, config in COINS.items():
        for window_size in WINDOW_SIZES:
            if experiment_type == "regular":
                tasks.append((symbol, config, window_size, 0, "regular"))
            elif experiment_type == "fullimage":
                tasks.append((symbol, config, window_size, 0, "fullimage"))
            elif experiment_type == "irregular":
                for missing_ratio in MISSING_RATIOS:
                    tasks.append((symbol, config, window_size, missing_ratio, "irregular"))
...
def run_all_experiments(experiment_type="regular"):
    if any(gpus):
        print(f"Running {experiment_type} experiments with GPU acceleration")
        run_all_experiments_parallel(experiment_type)
    else:
        print(f"Running {experiment_type} experiments in parallel CPU mode")
        run_all_experiments_parallel(experiment_type)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Merged Crypto Minute-Based Image Classification with Regular, Full Image, and Irregular Data")
    parser.add_argument("--experiment", choices=["regular", "fullimage", "irregular", "all"], default="regular", help="Type of experiment to run")
    ...
    if args.experiment == "all":
        print("Running all experiment types: regular, fullimage, and irregular")
        run_all_experiments("regular")
        run_all_experiments("fullimage")
        run_all_experiments("irregular")
    else:
        run_all_experiments(args.experiment)