#!/usr/bin/env python3
"""
Memory-efficient version of train_regular_pretrained.py with GPU support.
...
def process_coin(symbol, train_month, test_months, window_size, model_type, time_lengths=None, exp2_only=False):
    ...
    if not exp2_only:
        ...
        for days in TIME_LENGTHS_FILTERED:
            ...
            if os.path.exists(train_result):
                ...
                train_and_evaluate_model(labels_df, images_subdir, model_type, period_name, train_month_str, window_size, coin_dir, "train", "")
                del labels_df
                gc.collect()
            ...
            for test_year, test_month_num in test_months:
                ...
                if os.path.exists(test_result):
                    ...
                    labels_df = pd.read_csv(labels_file)
                    train_and_evaluate_model(labels_df, images_subdir, model_type, period_name, test_month_str, window_size, coin_dir, "test", "")
                    del labels_df
                    gc.collect()
    ...
    if os.path.exists(train_result):
        ...
        labels_file = generate_images(df, window_size, images_subdir, period_name, train_month_str, input_size)
        labels_df = pd.read_csv(labels_file)
        train_and_evaluate_model(labels_df, images_subdir, model_type, period_name, train_month_str, window_size, coin_dir, "train", "_exp2")
        del labels_df
        gc.collect()
    ...
    for test_year, test_month_num in test_months:
        ...
        for days in exp2_test_lengths:
            ...
            if os.path.exists(test_result):
                ...
                labels_file = generate_images(df, window_size, images_subdir, period_name, test_month_str, input_size)
                labels_df = pd.read_csv(labels_file)
                train_and_evaluate_model(labels_df, images_subdir, model_type, period_name, test_month_str, window_size, coin_dir, "test", "_exp2")
                del labels_df
                gc.collect()
