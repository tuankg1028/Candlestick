#!/usr/bin/env python3
"""
Memory-efficient version of train_irregular_pretrained.py with GPU support.
Dataset: Irregular (missing data: 60%, 80%, 95% omission)
Tests model robustness with incomplete data.

Key improvements:
- Uses TensorFlow Dataset and PyTorch DataLoader for lazy loading
- Added --time-length flag to process specific time periods
- Added --missing flag to specify missing ratio (0.6, 0.8, 0.95)
- Batch loading from disk instead of loading all images into RAM
- Reduced memory usage from ~24GB per dataset to ~500MB

Usage:
    # Process with 60% missing data
    python src/train_irregular_pretrained_gpu.py --model mobilenetv3 --coin BTCUSDT --window 5 --missing 0.6
...
# Missing data ratios for irregular dataset
...
class LazyImageDataset(Dataset):
    ...
    def __init__(self, labels_df, images_dir, input_size, transform=None):
...
def create_tf_dataset(labels_df, images_dir, input_size, batch_size=BATCH_SIZE_TF, shuffle=True):
    """Create a TensorFlow Dataset that loads images lazily from disk."""
    image_paths = labels_df["image_path"].values
    labels = labels_df["label"].values

    def load_image(image_path, label):
        img = tf.io.read_file(os.path.join(images_dir, image_path))
...
def create_pytorch_dataloader(labels_df, images_dir, input_size, batch_size=BATCH_SIZE_TORCH, shuffle=True):
    ...
    dataset = LazyImageDataset(labels_df, images_dir, input_size, transform=transform)
...
def fetch_coin_data(symbol, start_time, end_time, missing_ratio):
    """Fetch data from Binance and apply irregular data omission."""
    ...
    # Apply irregular data omission for irregular dataset
    if missing_ratio > 0:
        n_rows = len(df)
        ...
        print(f"Applied {missing_ratio*100}% data omission: kept {n_keep}/{n_rows} rows")

    return df[["timestamp", "open", "high", "low", "close"]]


def generate_images(df, window_size, output_dir, period_name, month_str, input_size, missing_ratio):
    """Generate images with irregular (missing) data patterns."""
    os.makedirs(output_dir, exist_ok=True)
    missing_pct = int(missing_ratio * 100)
    labels_file = os.path.join(output_dir, f"labels_{month_str}_1m_{period_name}_w{window_size}_{missing_pct}pct_size{input_size}.csv")
    ...
    # Ensure timestamp is index for windowing (may already be index)
    if "timestamp" in df.columns:
        df_indexed = df.set_index("timestamp")
    else:
        df_indexed = df

    original_timestamps = pd.date_range(start=df_indexed.index[0], end=df_indexed.index[-1], freq="1min")
    ...
    for i in range(len(original_timestamps) - window_size + 1):
        ...
        window_indices = df_indexed.index[(df_indexed.index >= window_start) & (df_indexed.index <= window_end)]
        window_df = df_indexed.loc[window_indices]

        if len(window_df) > 0:
            # Last candle labeling for irregular dataset
            ...
            image_path = os.path.join(output_dir, f"candle_{i}_{missing_pct}pct.png")
            ...
            if i % 1000 == 0:
                ...
                print(f"Generated image {i}/{len(original_timestamps) - window_size + 1} for {month_str} 1m {period_name} w{window_size} {missing_pct}% ({speed:.2f} images/sec)")
        else:
            continue

    # Create labels dataframe with actual generated images
    image_paths = [f"candle_{i}_{missing_pct}pct.png" for i in range(len(original_timestamps) - window_size + 1) if os.path.exists(os.path.join(output_dir, f"candle_{i}_{missing_pct}pct.png"))]
    labels_df = pd.DataFrame({"image_path": image_paths, "label": labels[:len(image_paths)]})
...
def train_and_evaluate_model(labels_df, images_dir, model_type, period_name, month_str, window_size, coin_dir, dataset_type="train", exp_suffix=""):
    ...
    if model_config['framework'] == 'tensorflow':
        ...
        dataset = create_tf_dataset(labels_df, images_dir, input_size, batch_size=BATCH_SIZE_TF, shuffle=(dataset_type == "train"))
        ...
        dataloader = create_pytorch_dataloader(labels_df, images_dir, input_size, batch_size=BATCH_SIZE_TORCH, shuffle=(dataset_type == "train"))
...
def process_coin(symbol, train_month, test_months, window_size, model_type, missing_ratio, time_lengths=None, exp2_only=False):
    """Process irregular dataset with specified missing data ratio."""
    missing_pct = int(missing_ratio * 100)
    print(f"\nProcessing {symbol} with {model_type} model - window size {window_size} - {missing_pct}% missing data")
    ...
    if not exp2_only:
        ...
        for days in TIME_LENGTHS_FILTERED:
            ...
            # Check if train result already exists (include missing_pct in filename)
            train_result = os.path.join(RESULTS_DIR, f"results_{model_type}_train_{train_month_str}_1m_{period_name}_w{window_size}_{missing_pct}pct.txt")
            if os.path.exists(train_result):
                ...
                # For irregular: don't cache raw data since missing_ratio is random, or include missing_pct in filename
                raw_file = os.path.join(RAW_DATA_DIR, f"raw_{train_month_str}_1m_{period_name}_{missing_pct}pct.csv")
                ...
                labels_file = generate_images(df, window_size, images_subdir, period_name, train_month_str, input_size, missing_ratio)
                if labels_file is None:
                    print(f"Skipping {period_name} due to insufficient data")
                    continue
                labels_df = pd.read_csv(labels_file)
                train_and_evaluate_model(labels_df, images_subdir, model_type, period_name, train_month_str, window_size, coin_dir, "train", f"_{missing_pct}pct")
                del labels_df
                gc.collect()

            for test_year, test_month_num in test_months:
                test_month_str = f"{test_year}-{test_month_num:02d}"
                test_result = os.path.join(RESULTS_DIR, f"results_{model_type}_test_{test_month_str}_1m_{period_name}_w{window_size}_{missing_pct}pct.txt")
                ...
                if os.path.exists(test_result):
                    ...
                    raw_file = os.path.join(RAW_DATA_DIR, f"raw_{test_month_str}_1m_{period_name}_{missing_pct}pct.csv")
                    if not os.path.exists(raw_file):
                    ...
                    labels_file = generate_images(df, window_size, images_subdir, period_name, test_month_str, input_size, missing_ratio)
                    if labels_file is None:
                        print(f"Skipping {period_name} due to insufficient data")
                        continue
                    labels_df = pd.read_csv(labels_file)
                    train_and_evaluate_model(labels_df, images_subdir, model_type, period_name, test_month_str, window_size, coin_dir, "test", f"_{missing_pct}pct")
                    del labels_df
                    gc.collect()
    ...
    train_result = os.path.join(RESULTS_DIR, f"results_{model_type}_train_{train_month_str}_1m_{period_name}_w{window_size}_exp2_{missing_pct}pct.txt")
    if os.path.exists(train_result):
        ...
        raw_file = os.path.join(RAW_DATA_DIR, f"raw_{train_month_str}_1m_{period_name}_{missing_pct}pct.csv")
        if not os.path.exists(raw_file):
            df = fetch_coin_data(symbol, train_start, end_time=train_end, missing_ratio=missing_ratio)
            df.set_index("timestamp", inplace=True)
            df.to_csv(raw_file)
            print(f"Raw data saved to {raw_file}")
        else:
        ...
        if labels_file is None:
            ...
            labels_df = pd.read_csv(labels_file)
            train_and_evaluate_model(labels_df, images_subdir, model_type, period_name, train_month_str, window_size, coin_dir, "train", f"_exp2_{missing_pct}pct")
            del labels_df
            gc.collect()
    ...
    for test_year, test_month_num in test_months:
        ...
        for days in exp2_test_lengths:
            period_name = f"{days}days"
            test_result = os.path.join(RESULTS_DIR, f"results_{model_type}_test_{test_month_str}_1m_{period_name}_w{window_size}_exp2_{missing_pct}pct.txt")
            ...
            if os.path.exists(test_result):
                ...
                if labels_file is None:
                    ...
                    continue
                labels_df = pd.read_csv(labels_file)
                train_and_evaluate_model(labels_df, images_subdir, model_type, period_name, test_month_str, window_size, coin_dir, "test", f"_exp2_{missing_pct}pct")
                del labels_df
                gc.collect()


def main():
    parser = argparse.ArgumentParser(description="Memory-efficient irregular dataset training with GPU support")
    ...
    parser.add_argument("--missing", type=float, nargs="+", choices=[0.6, 0.8, 0.95], help="Missing ratio(s) - 0.6, 0.8, or 0.95 (default: all)")
    ...
    # Determine missing ratios to run
    if args.missing:
        missing_ratios = args.missing
    else:
        missing_ratios = MISSING_RATIOS

    print(f"\n{'='*60}")
    print(f"Memory-Efficient Training on IRREGULAR dataset")
    print(f"Models: {', '.join(models_to_run)}")
    print(f"Coins: {', '.join(coins.keys())}")
    print(f"Windows: {window_sizes}")
    print(f"Missing ratios: {[int(r*100) for r in missing_ratios]}%")
    if time_lengths:
        ...
        for model_type in models_to_run:
            for window_size in window_sizes:
                for missing_ratio in missing_ratios:
                    ...
                    missing_pct = int(missing_ratio * 100)
                    print(f"\n[{completed}/{total_jobs}] {pct:5.1f}% | ETA: {eta/60:.1f}m | {symbol} - {model_type} - w{window_size} - {missing_pct}% missing")
                    print("-" * 60)
                    try:
                        process_coin(symbol, train_month, test_months, window_size, model_type, missing_ratio, time_lengths, args.exp2_only)
