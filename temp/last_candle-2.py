...
def generate_images_batch(df, window_size, output_dir, period_name, month_str, batch_size=100):
    os.makedirs(output_dir, exist_ok=True)
    labels_file = os.path.join(output_dir, f"labels_{month_str}_1m_{period_name}_w{window_size}.csv")
    if os.path.exists(labels_file):
        print(f"Labels already exist at {labels_file}, skipping image generation")
    ...
    for batch_start in range(0, total_images, batch_size):
        ...
        for i in range(batch_start, batch_end):
            ...
            plt.tight_layout(pad=0)
            image_path = os.path.join(output_dir, f"candle_{actual_index}.png")
            plt.savefig(image_path, bbox_inches="tight", pad_inches=0, dpi=24)
            plt.close(fig)
    ...
    labels_df = pd.DataFrame({
        "image_path": [f"candle_{i + window_size - 1}.png" for i in range(total_images)], 
        "label": labels
    })
    labels_df.to_csv(labels_file, index=False)
    print(f"Saved {len(labels_df)} labels to {labels_file}")
...
# Optimized image loading with multiprocessing
...
# Optimized model training with mixed precision and callbacks
def train_model_optimized(X, y, period_name, month_str, window_size, coin_dir):
    model_path = os.path.join(coin_dir, "models", f"model_{month_str}_1m_{period_name}_w{window_size}.h5")
    if os.path.exists(model_path):
        print(f"Model already exists at {model_path}, loading instead of training")
        return tf.keras.models.load_model(model_path), None
...
def run_experiments_for_coin(symbol, train_month, test_months, window_size):
    ...
    if not os.path.exists(raw_file):
        ...
        # df.to_csv(raw_file)
        # print(f"Raw data saved to {raw_file}")
        print(f"Raw data file {raw_file} does not exist, skipping Experiment II")
        return
    else:
        print(f"Raw data already exists at {raw_file}, skipping fetch")
        df = pd.read_csv(raw_file, index_col="timestamp", parse_dates=["timestamp"])
...
# Parallel experiment runner for single coin
def run_single_coin_experiment(args):
    symbol, config, window_size = args
    try:
        run_experiments_for_coin(symbol, config["train_month"], config["test_months"], window_size)
        return f"Completed {symbol} w{window_size}"
    except Exception as e:
        return f"Error in {symbol} w{window_size}: {str(e)}"

# Parallel execution for all experiments
def run_all_experiments_parallel():
    ...
    for symbol, config in COINS.items():
        for window_size in WINDOW_SIZES:
            tasks.append((symbol, config, window_size))
    
    # Determine optimal number of workers
    max_workers = min(psutil.cpu_count() // 2, 25)  # Conservative approach
    print(f"Running {len(tasks)} experiments with {max_workers} parallel workers")
    
    # Use ThreadPoolExecutor for I/O bound operations
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_task = {executor.submit(run_single_coin_experiment, task): task for task in tasks}
